...
if(isset($_GET["type"])){
	if(strtolower($_GET["type"])=="atom"){
		...
		$rss_node->appendChild($xml->createElement("title", "WePic"));
		$rss_node->appendChild($xml->createElement("subtitle", "Tus imagenes, cuando quieras, donde quieras."));
		$link_node = $rss_node->appendChild($xml->createElement("link"));
		$link_node->setAttribute("href","http://localhost/feed.php?type=atom");
		$link_node->setAttribute("rel","self");
		$link2_node = $rss_node->appendChild($xml->createElement("link"));
		$link2_node->setAttribute("href","http://localhost");
		...
		if ($res){
			while ($row = $res->fetch_assoc()){
				...
				$link_node->setAttribute("href","http://localhost/detalle.php?id=".$row["id"]);
				...
				$link2_node->setAttribute("href","http://localhost/detalle.php?id=".$row["id"]);
				...
				//Published date
				$date_rfc = gmdate(DATE_RFC3339, strtotime($row["fechaSubida"]));
		...
		$channel_node->appendChild($xml->createElement("title", "WePic"));
		$channel_node->appendChild($xml->createElement("description", "Tus imagenes, cuando quieras, donde quieras."));
		$channel_node->appendChild($xml->createElement("link", "http://localhost"));
		...
		$channel_atom_link->setAttribute("href","http://localhost/feed.php?type=atom"); //url of the feed
		...
		if ($res){
			while ($row = $res->fetch_assoc()){
			    ...
			    $title_node = $item_node->appendChild($xml->createElement("title", $row["titulo"])); //Add Title under "item"
			    $link_node = $item_node->appendChild($xml->createElement("link", "http://localhost/detalle.php?id=".$row["id"])); //add link node under "item"

				//Unique identifier for the item (GUID)
			    $guid_link = $xml->createElement("guid", "http://localhost/detalle.php?id=".$row["id"]);
