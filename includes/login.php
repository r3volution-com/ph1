<section class="box">
	...
	<form method="POST" action="operaciones.php?operacion=login">
		...
		<input id="nombre" name="nombre" minlength="3" maxlength="15" pattern="[a-zA-Z0-9\s]{3,15}" type="<?php if(!isset($_COOKIE["remember_user"])) echo "text"; else echo "hidden";?>" placeholder="Nombre" value="<?php if(isset($_COOKIE["remember_user"])) echo $_COOKIE["remember_user"];?>"/><br>
		<label for="pass" class="hide">Contraseña</label>
		<input id="pass" name="pass" minlength="6" maxlength="15" pattern="[a-zA-Z0-9\s_]{6,15}" type="<?php if(!isset($_COOKIE["remember_user"])) echo "password"; else echo "hidden";?>" placeholder="Contraseña" value="<?php if(isset($_COOKIE["remember_pass"])) echo $_COOKIE["remember_pass"];?>"/><br>
